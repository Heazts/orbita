...
export async function GET(request: NextRequest) {
  ...
  // rateHeaders isn't included here: this response is cached at the edge for
  // up to 5 minutes (s-maxage=300) and served to many different callers, so a
  // per-caller X-RateLimit-Remaining baked into it would be stale/misleading
  // for everyone but the one request that generated the cached copy. The 429
  // path above is never cached, so its rate-limit headers stay accurate.
