...
- **Imagens externas**: extraídas apenas de URLs `https://` e servidas pelo proxy próprio (`/api/img-proxy`), para que os veículos não recebam o IP de quem lê. O servidor buscar uma URL de terceiro é uma superfície de SSRF, e é por isso que o proxy é escrito à mão em vez de usar o otimizador do Next: ele resolve o DNS, recusa endereços privados/reservados (incluindo formas IPv4 mapeadas em IPv6 e 6to4), **conecta no endereço já validado** em vez de reentregar o nome à pilha de rede (fecha DNS rebinding), exige HTTPS na porta 443, limita redirects, tamanho e tipos MIME (SVG não entra), e é medido por IP. `referrerPolicy="no-referrer"` evita vazar a origem do site. Coberto por `tests/safe-remote-url.test.ts` e `tests/img-proxy.test.ts`.
