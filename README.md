...
- **Rate limiting**: toda rota alcançável por um visitante é medida, cada uma no seu próprio balde por IP (`lib/rate-limit.ts`), para que carregar as miniaturas de uma página não consuma o orçamento de busca do mesmo leitor: `/api/news` e `/api/health` a ~30/min, `/api/img-proxy` a 240/min (uma lista cheia pede até 100 imagens) e `/api/csp-report` a 60 requests/min e 60 linhas de log/min. Requisições já rejeitadas não aumentam o estado do fallback local. `tests/privacy-claims.test.ts` varre `app/api/` e falha se uma rota nova aparecer sem limite. É best-effort, em memória por instância, ou distribuído entre instâncias via Upstash quando configurado. **A chave é o IP dos cabeçalhos `x-real-ip`/`x-forwarded-for`, o que só é confiável porque a Vercel os reescreve na borda** — em outra hospedagem que os repasse intactos, o limite é contornável; veja o comentário em `clientIp()`.
- **Sem SSRF**: a rota `/api/news` só faz `fetch` para uma lista fixa de feeds (`FEED_SOURCES`) e para o domínio fixo `news.google.com`; a entrada do usuário (`q`) é sempre passada como parâmetro de URL codificado, nunca como host/URL arbitrário. Redirects de feeds e APIs financeiras são manuais, limitados e só podem permanecer na mesma origem HTTPS.
...
- **Imagens externas**: extraídas apenas de URLs `https://` e servidas pelo proxy próprio (`/api/img-proxy`), para que os veículos não recebam o IP de quem lê. O agregador assina com HMAC somente as URLs encontradas nos feeds configurados (`IMAGE_PROXY_SECRET`, com fallback domain-separated para `CRON_SECRET`), por isso a rota não funciona como proxy genérico para URLs escolhidas pelo visitante. O servidor resolve o DNS, recusa endereços privados/reservados (incluindo formas IPv4 mapeadas em IPv6 e 6to4), **conecta no endereço já validado** em vez de reentregar o nome à pilha de rede (fecha DNS rebinding), exige HTTPS na porta 443, limita redirects, tamanho e tipos MIME (SVG não entra), e aborta imediatamente respostas rejeitadas. `referrerPolicy="no-referrer"` evita vazar a origem do site. Coberto por `tests/safe-remote-url.test.ts`, `tests/image-proxy-signature.test.ts` e `tests/img-proxy-response.test.ts`.
